...
export async function initLocalProgress() {
  ...
  bar.className = 'local-progress'; bar.setAttribute('aria-label', '本地进度');
  ...
  try {
    ...
    if (disk.payload && meaningful(browser) && browserPrint !== fingerprint(disk.payload) && await checksum(browserPrint) !== base) {
      label.textContent = '浏览器与进度文件不同。请先导出备份，再选择本次使用的数据：';
      save.hidden = true;
        ...
        for (const [text, useDisk] of [['使用文件中的进度', true], ['将浏览器进度保存到文件', false]]) {
          ...
          button.onclick = () => {
            try {
              ...
              resolve();
            } catch (error) { label.textContent = `恢复失败：${error.message}。原数据已保留，请先导出备份。`; }
          };
          bar.append(button);
        }
      });
    ...
    if (fingerprint(exportAll()) !== last) await persist();
      ...
      await remember();
      label.textContent = firstVisit ? '本地保存已就绪 · 开始练习后自动写入文件' : '本地进度已载入 · 修改后自动保存';
    }
  ...
  } catch (error) { label.textContent = `当前仅保存在浏览器：${error.message}`; save.disabled = true; }
}
